...
function App() {
  ...
  React.useEffect(() => {
    if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
      ...
      recognition.lang = language; // Set the language based on state
      recognition.interimResults = false;
      recognitionRef.current = recognition;
    } else {
      console.warn('Speech Recognition API not supported in this browser.');
    }
  }, [language]); // Reinitialize the recognition when language changes
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100">
      ...
      <div className="p-4 bg-gray-800 border-b border-gray-700">
        <label htmlFor="language-select" className="text-gray-300 mr-2">Select Language:</label>
        <select
          id="language-select"
          value={language}
          onChange={(e) => setLanguage(e.target.value)}
          className="bg-gray-700 text-white border border-gray-600 px-3 py-2 rounded-lg"
        >
